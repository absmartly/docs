...
The **metric catalog** is now accessible directly from the main menu, highlighting the central role metrics play across the platform.
...
### CUPED Support  
Run experiments faster and detect smaller effects with **[CUPED (Controlled-experiment Using Pre-Experiment Data)](/docs/web-console-docs/goals-and-metrics/metrics/variance-reduction-cuped)**.  
...
- Only **active** metrics can be used in experiments  
...
We've also improved the historgram graph so now the buckets match making comparison between variants much easier.
