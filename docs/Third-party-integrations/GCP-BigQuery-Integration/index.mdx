...
When using a single table, filter by experiment ID to get data for a specific
experiment:

```sql
-- Get all data for experiment 42
SELECT *
...
When using one table per experiment, each table is named
`experiment_{id}_exports`:
