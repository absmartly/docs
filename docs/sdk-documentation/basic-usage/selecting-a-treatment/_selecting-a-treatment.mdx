...
import ReactUsingReactFragments from "!!raw-loader!./react/usingReactFragments.js";
...
component. The children of the component will be the different variants that
can be displayed.

The first child of the component is the default variant (_Variant 0_ - the
control group) and each following child is a new variant with an incremented
index.
...
If you want to have multiple components in each variant, you can use
React Fragments to contain them.

<CodeBlock language="jsx">{ReactUsingReactFragments}</CodeBlock>
