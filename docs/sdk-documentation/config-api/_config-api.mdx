...
API. When the config API is used, you don't need to call the `treatment()` method.
...
the second one. Calling `context.mergeConfig(cfg)` from the Javascript SDK would
...
<CodeBlock language="js">{VueConfigParameters}</CodeBlock>
...
<CodeBlock language="js">{VueReturnedConfig}</CodeBlock>
