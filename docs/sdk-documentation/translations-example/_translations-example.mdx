...
export default
// Copy text for English and Dutch
{
    header: {
        en: "Our nice header!",
        nl: "Onze leuk kop!",
...
```js
// Create a function to fetch the variant translations and return them as an object.
const fetchTranslations = (keysArray) => {
    return keysArray.reduce(
        (acc, key) => (
            { ...acc, [key]: context.variableValue(key) }
        )
    )
}

// Get the variant variable keys from the context
