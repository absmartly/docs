...
### A/A experiment  
**Definition:** An experiment where users are randomly split between two (or more) *identical* variants. The goal is not to test a product change, but to validate the experimentation setup itself.  
**Why it matters:** A/A experiments help detect issues such as sample ratio mismatch (SRM), tracking bugs or unexpected bias in randomisation before you start testing real changes. :contentReference[oaicite:0]{index=0}  
...
**Why it matters:** A/B experiments are the core building block of product experimentation and allow teams to quantify how a change affects key metrics. :contentReference[oaicite:1]{index=1}  
...
### A/B/n experiment  
**Definition:** An experiment that compares a control to *multiple* treatments at the same time (A vs B vs C, etc.).  
**Why it matters:** A/B/n experiments speed up exploration when several ideas are available, but they increase the number of comparisons and therefore require more traffic or careful error control. :contentReference[oaicite:2]{index=2}  
...
### Audience targeting  
**Definition:** The practice of restricting an experiment or a variant to a specific subset of users based on attributes, behaviour or context.  
**Why it matters:** Targeting ensures that experiments are run on the right population (for example, new users only), but aggressive targeting can reduce sample size and affect generalisability. :contentReference[oaicite:3]{index=3}  
**Example:** Running an experiment only for traffic from a specific country or only for logged-in customers.

### Binomial  
**Definition:** A statistical model describing outcomes that have exactly two states such as success / failure or convert / not convert.  
**Why it matters:** Many core metrics in experimentation, like conversion rate, follow a binomial process and use binomial-based methods for confidence intervals and tests. :contentReference[oaicite:4]{index=4}  
...
**Why it matters:** Behavioural metrics such as clicks, scroll depth or page views are often more sensitive and can explain *why* a business metric moves.  
...
### Conversion rate  
**Definition:** The proportion of users who complete a defined goal out of all eligible users.  
**Why it matters:** Conversion rate is one of the most common primary metrics, and small changes in conversion can have large business impact.  
...
**Why it matters:** Confidence intervals convey both the size and the uncertainty of an effect, which is more informative than a p-value alone. :contentReference[oaicite:5]{index=5}  
**Example:** An observed lift of 5 percent with a 95 percent confidence interval from 2 percent to 8 percent.

### Confirmation bias  
**Definition:** The tendency to focus on data that supports pre-existing beliefs and ignore or downplay conflicting evidence.  
...
### Confidence level  
**Definition:** The probability that the confidence interval procedure will capture the true value, across many hypothetical repetitions of the experiment.  
**Why it matters:** Common choices such as 95 percent or 99 percent define how strict you are about uncertainty and directly relate to the significance level.  
**Example:** Using a 95 percent confidence level means accepting a 5 percent chance that the interval misses the true effect.
