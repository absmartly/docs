...
One-sided tests offer greater statistical power than two-sided tests, meaning they can detect effects with smaller sample sizes but only when you care about a change in one direction.

When it's appropriate:
One-sided analysis makes sense if:
...
**Example:** You test a new pricing design. If it improves revenue, you’ll ship it. If it’s flat or worse, you won’t — so a one-sided test for improvement is appropriate.

### Operational metric  
A metric that reflects system health or performance rather than direct user or business outcomes.  
While some experiment might be targeting them, perational metrics often act as guardrails and basic safety checks during experiments.  
...
Uncontrolled peeking increases the chance of false positives and leads to unreliable decisions. 
