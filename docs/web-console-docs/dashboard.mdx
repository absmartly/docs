...
The "Code Cleanup Needed" alert reminds you to clean up your
...
The "Audience Mismatch" alert tells you that your experiment
...
with 2 variants and a 50/50 split, but Variant A had 1,000 impressions and
Variant B had 2,000 impressions, a sample ratio mismatch would be flagged.
...
###### Traffic Allocation Warnings

<Image
    img="traffic_allocation_info_alert.png"
    alt="An example of the traffic allocation info
alert."
/>
<Image
    img="traffic_allocation_warning_alert.png"
    alt="An example of the traffic allocation warning
alert."
/>

The "Traffic Allocation" alert either informs you that your traffic 
allocation is below 100% or warns you that it is set to 0%. Having a 
traffic allocation of 0% means that no participants will be exposed
to your experiment. 

###### Variable Conflicts

<Image
    img="variables_conflicting_alert.png"
    alt="An example of the conflicting variables warning
alert."
/>

A variable conflict is when you have a variable key that is being used in more
than one experiment. This can cause issues when pulling variable values from ABsmartly
in your code.

##### Health Checks

<Image img="health_checks_alert.png" alt="An example of the health checks 
alert. It says, 3 out of 3 health checks completed - 2 alerts found." />

The health checks section allows you to see the status of the various checks
that ABsmartly runs on your experiments to generate the alerts mentioned above.

If you click on the health checks alert, you will be taken to the health checks
panel, where you can see the details of the health checks that have been run.

<Image img="health_checks_panel.png" alt="An example of the health checks 
panel. In it, a sample ratio mismatch and variable conflicts have been 
detected." 
  maxWidth="25rem"
/>
...
In the explore tab, you can filter and see information about how your 
metrics have performed during an experiment (Read more about metrics in the 
[creating a metric docs](/docs/web-console-docs/tutorial#creating-a-metric)).
Metrics can be filtered by segment, by time period and can display the 
data based on the first exposure or the last exposure of the participant.
Once you have selected a metric, you can see the data in the summary, graphs
and segments tabs.
...
<Image
    img="segments.png"
    alt="A screenshot of the segments tab of an experiment. It has a table with
    data telling us that the new Variant is outperforming the control group"
    maxWidth="45rem"
/>

In the example above, our "Returned for purchase" metric has been
positively impacted by our variant, but when we look at the segments
table, we find that users who speak Spanish or Dutch are much more 
impacted than users who speak other languages.
...
<Image
    img="experiment_activity_tab.png"
    alt="A screenshot of the activity tab of an experiment. It has a note that the experiment was started and a report of the experiment's outcome."
    maxWidth="45rem"
/>
...
When an alert is triggered, it will also be shown in the activity tab 
allowing for easy tracking and commenting.

<Image
    img="activity_alert.png"
    alt="A screenshot of the activity tab of an experiment. It has a note that the experiment was started and a report of the experiment's outcome."
    maxWidth="45rem"
/>
