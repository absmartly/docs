...
To ensure an optimal Group Sequential analysis, it is critical that the metric's variance and standard deviation have stabilized at the moment of the first analysis (you can see that when the graph flattens out).
...
As mentioned above, it is important those numbers are stabilized by the time this analysis takes place otherwise the interim analysis setup will not be as precise.
...
Running an analysis too early in the experiment could lead to significant results which are biased and not representative of the entire population (for example by having a non-representative sample of visitors, too many frequent users, too many week-end users, etc).
...
Too many interim analyses lead to a decrease in power so longer runtime on average.
Too few interim analyses lead to less agility and velocity to the program.
Defining how often interim analyses should run is key but it's also very depend on the context and might require a few tries. 
:::
...
:::info
Because they are one-sided, GST experiment will always cross either the futility or the efficiency boundary at the end of the runtime.
:::
