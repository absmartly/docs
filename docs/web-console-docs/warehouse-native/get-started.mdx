...
In Hybrid mode, a goal can live in an external warehouse, but exposures come from the default data source (ABsmartly's managed ClickHouse). To compute metrics for such a goal, ABsmartly needs to join those exposures against the goal — which lives in a different warehouse. It bridges the gap by exporting the exposures (participants) table as **Parquet** to an object store, then importing it into your external data source at query time.
