...
#### Reporting period
On top of the report users must select the period over which they want to report. By default the reporting period is set to the last 30 days.
...
In the `iterations` this experiment would be counted twice `started` while in the `experiments` view it would only appear once. The last instance wiould be the one visualised on the graphs.

#### Sharing
It is possible to share an exact view of the report by clicking on the share button on the right side of the report.
...
The report also visualise the result of the statistical test at the moment the experiment was completed. 
The experiment will be shown in `green` if its `primary metric is conclusive in the expected direction` at the time the experiment is completed.
...
or because of early worrying negative signals or any other reason. Read our [When to abort an experiment? guide](Aborting-experiments) to understand more about aborting experiments.

:::caution
Aborted experiments do not provide reliable evidence on which to base decisions.
Depending on the reason for aborting, aborted experiment are often restarted once the underlying issue is fixed.
:::
...
This provides a good overview of the quality of the experimentation program and decisons which are based on experimentation.