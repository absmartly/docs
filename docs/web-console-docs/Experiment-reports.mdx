...
For a [Group Sequential Test](types-of-analysis), an experiment is completed once it crosses one of the test boundary (efficiency or futility).
...
provides valuable evidence and insights on which to base decisions, and in that sense they should all be seen as success. 
Knowning that something does not have the expected impact on customer behaviour can be as important and insightful as validating that something else does.
:::
...
Depending on the reason for aborting, aborted experiments are typically restarted once the underlying issue is fixed.
:::
