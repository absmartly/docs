...
:::note Check the `flags` column before you analyse exposures
An export contains every matching exposure within your export scope, not just the
real participants. The [`flags` field](exposure-events#the-flags-field) is what
separates the two. Skip it and your participant counts come out too high, and the
same visitor can end up on more than one variant.
:::
