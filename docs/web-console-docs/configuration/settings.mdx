...
:::info Looking for a personal API key?
These application-level API keys are different from the personal API keys each user can create
for themselves, for example to use with the [ABsmartly CLI](/docs/APIs-and-SDKs/CLI-Documentation/authentication).
See [Personal API Keys](/docs/web-console-docs/users-teams-permissions/personal-api-keys) for how
to create one from your user profile.
:::
...
Have a look at our [Slack integration guide](/docs/APIs-and-SDKs/Web-Console-API/examples/slack-integration) for an example of how to use Webhooks,
...
#### Setting Custom Field Values via the API

When creating or updating an experiment through the API rather than the Web Console, `custom_section_field_values`
must be sent as an object keyed by each field's numeric id, not as an array:

```json
{
  "data": {
    "custom_section_field_values": {
      "75": { "type": "boolean", "value": "false" },
      "79": { "type": "number", "value": "414" }
    }
  }
}
```

The [ABSmartly CLI](/docs/APIs-and-SDKs/CLI-Documentation) builds this shape for you automatically — see
[custom field options](/docs/APIs-and-SDKs/CLI-Documentation/experiments-and-feature-flags#custom-field-options) and
[`--show-request`](/docs/APIs-and-SDKs/CLI-Documentation/configuration#debugging-api-traffic) if you want to inspect
the exact payload for a working request.
