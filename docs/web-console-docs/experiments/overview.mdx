...
ABsmartly uses sound statistical methods to compute the impact of each variant compared to control. 
...
ABsmartly supports both server-side and client-side experimentation seamlessly. 
Each approach has different strengths depending on where the change is implemented and how data flows through your system.
...
- Non-engineers can create visually in the [Visual Editor](../launchpad-browser-extension/getting-started)
...
- experiment setup can no longer be edited (some metadata can still be edited)
