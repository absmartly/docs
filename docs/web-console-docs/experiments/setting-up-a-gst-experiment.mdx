...
To understand the differences between Group Sequential and Fixed Horizon experiment check our [analysis type article](overview#analysis-methods).
...
A 90% confidence in a 2-sided analysis (like we use in Fixed Horizon), means a 5% false positive on each side of the interval (5% of the false positive will be significantly positive and 5% will be significantly negative)
...
In practice time between analysis will probably be larger than the specific minimum interval as it is dependant on many factors,
 for example, the maximum number of possible analyses (currently 11), MDE, daily visitors counts, metric variance, etc.
