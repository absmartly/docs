...
[treatment variables](/docs/APIs-and-SDKs/SDK-Documentation/running-your-first-experiment#step-2-use-variables-for-flexible-experiments) in the SDK Documentation for more information.
...
These audience parameters can then be passed to the SDK in your code using [context attributes](/docs/APIs-and-SDKs/SDK-Documentation/targeting-and-audiences#passing-user-attributes-for-targeting).
#### Audience Enforced
...
1. **Who to exclude** — A set of conditions that identify the visitors to exclude, based on [context attributes](/docs/APIs-and-SDKs/SDK-Documentation/targeting-and-audiences#passing-user-attributes-for-targeting) (e.g. `user_type = internal`, `email ends_with @yourcompany.com`).
2. **Which variant to show** — The variant that excluded visitors will always be assigned to.
...
You can learn more about experimentation metrics in our [Understanding Experimentation Metrics](/docs/web-console-docs/goals-and-metrics/metrics/overview) article.
### The Primary Metric
...
:::tip
If you observe a significant impact on the primary metric but no significant differences on the secondary metrics then 
the impact observed on the the primary metric is likely a false positive. For more information, check our article on interpreting results and making decisions.
:::
...
To choose between a Group Sequential or Fixed Horizon analysis check our [analysis type article](overview#analysis-methods).
