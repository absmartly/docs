...
In the case of a [Fixed Horizon Experiment](overview#analysis-methods), it is deemed completed once it reaches its expected sample size or planned duration.
For a [Group Sequential Test](overview#analysis-methods), an experiment is completed once it crosses one of the test boundaries (efficiency or futility).
...
:::caution
The supported by evidence validation does not currently include checks on the secondary and guardrail metrics nor on the experiment [health checks](experiment-health-checks).
:::
...
:::caution
Where an experiment has no confidence interval data available for the selected metric, a warning is shown above the report indicating how many experiments are affected.
Their point estimates remain included in the totals, but they contribute no uncertainty to the likely range, which will therefore appear narrower than it should be.
:::
...
<Image img="experiment-report/impact-over-time-tooltip.png" alt="Impact over time chart tooltip" maxWidth="50rem" />
