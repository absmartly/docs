...
| **Access Key**  | The access key id of for the object storage bucket.                           |
| **Secret Key**  | The secret access key for the object storage bucket.                          |
| **Endpoint**    | The S3-compatible endpoint to connect to the object storage bucket. **Examples:**<br /> AWS: https://s3.amazonaws.com<br />GCP: https://storage.googleapis.com<br />See [this page](https://cloud.google.com/storage/docs/interoperability) about using GCP as S3 compatible storage.|
| **Format**      | The format of the exported data. CSV and TSV include a header row with column names. JSON is a newline-delimited JSON file.       |
| **Compression** | The compression method to use. GZIP is widely supported and provides good compression. LZ4 is much faster but provides slightly less compression. None is no compression.|
