...
<Image img="experiment-results/fh-positive-result.png" alt="Experiment result for a Fixed Horizon test in ABsmartly" />
...
\[ 
\text{Mean} = \frac{\text{Value}}{\text{Participants}} 
\]  
...
<Image img="experiment-results/fh-power-96-1.png" alt="Experiment power at 96.1%" />
...
- **Mean**: The expected average performance of the metric for the variant.
- **Observed Mean**: The actual observed average performance of the metric during the experiment.
- **Impact**: The percentage change in the metric compared to the baseline. In this example +1.74% with a confidence interval going from -1.88% to +5.49%.
