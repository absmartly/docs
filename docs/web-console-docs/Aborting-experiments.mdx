...
Stopping an experiment early —or **aborting**— are sometimes necessary, but it must be approached cautiously, especially when using fixed-horizon or group sequential testing (GST) engines. Each engine type has its own rules and considerations for early stopping. This article explains when it might be appropriate to stop or abort an experiment, the risks involved, and best practices for decision-making.

## Aborting fixed-horizon experiments

In a **fixed-horizon experiment**, results should ideally be evaluated only at the pre-determined horizon (e.g., after collecting the required sample size). Early stopping is not recommended for significant positive results because fixed-horizon tests require full sample size completion to ensure validity. However, there are rare exceptions where early stopping or **aborting** might be justified:

### Negative results with a potential high cost
If the treatment causes significant harm to **primary** or **guardrail metrics**, such as decreased conversion rates or increased error rates, it may justify stopping the experiment. However, this decision should be seen as a **precautionary measure** rather than conclusive evidence, as:
...
- **Stopping**: If the experiment reaches an interim analysis and crosses a pre-defined statistical boundary (positive or negative), it is stopped. This is not considered "early stopping" because the interim analysis was planned in advance. In this case the experiment is completed.
- **Aborting**: Stopping an experiment **before** reaching the next interim analysis due to unforeseen circumstances (e.g., bugs or severe negative impacts) is considered early stopping in GST.
