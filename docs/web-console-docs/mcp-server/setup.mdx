...
One-click install (OAuth): <BrowserOnly>{() => <a href={cursorInstallUrl}>Add to Cursor</a>}</BrowserOnly>. Cursor will prompt you for the ABsmartly instance URL in the browser, then the flow completes automatically.
...
One-click install (OAuth): <BrowserOnly>{() => <><a href={vscodeInstallUrl}>Install in VS Code</a> · <a href={vscodeInsidersInstallUrl}>Install in VS Code Insiders</a></>}</BrowserOnly>. You'll be prompted for your ABsmartly instance URL in the browser.
